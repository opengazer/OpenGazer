...
CalTarget::CalTarget(Point point, const IplImage *image, const IplImage *origImage):
	point(point),
...
GazeTracker::GazeTracker():
	...
	_nnEye(cvCreateImage(cvSize(16, 8), 8, 1)),
	_inputCount(0),
	_inputCountLeft(0)
{
}
...
void GazeTracker::addExemplar(Point point, const IplImage *eyeFloat, const IplImage *eyeGrey) {
	_calTargets.push_back(CalTarget(point, eyeFloat, eyeGrey));
	updateGaussianProcesses();
}

void GazeTracker::addExemplarLeft(Point point, const IplImage *eyeFloat, const IplImage *eyeGrey) {
	_calTargetsLeft.push_back(CalTarget(point, eyeFloat, eyeGrey));
	updateGaussianProcessesLeft();
}

void GazeTracker::addSampleToNN(Point point, const IplImage *eyeFloat, const IplImage *eyeGrey) {
	...
	cvResize(eyeGrey, _nnEye);
	//cvEqualizeHist(_nnEye, _nnEye);
	...
	for (int i = 0; i < _nnEyeWidth * _nnEyeHeight; ++i) {
		inputs[i] = (float)(_nnEye->imageData[i] + 129) / 257.0f;
...
void GazeTracker::addSampleToNNLeft(Point point, const IplImage *eyeFloat, const IplImage *eyeGrey) {
	// Save the entire grey image for later use
	IplImage *savedImage = cvCreateImage(cvSize(64, 32), IPL_DEPTH_32F, 1);
	cvCopy(eyeFloat, savedImage);
...
void GazeTracker::updateLeft(const IplImage *image, const IplImage *eyeGrey) {
	if (isActive()) {
