...
void EyeExtractor::extractEye(const IplImage *originalImage) throw (TrackingException) {
	...
	cvGetQuadrangleSubPix(originalImage, eyeImage.get(), &M);
	cvCvtColor(eyeImage.get(), eyeGrey.get(), CV_RGB2GRAY);
}

void EyeExtractor::extractEyeLeft(const IplImage *originalImage) throw (TrackingException) {
	//if (!_pointTracker.status[_pointTracker.eyepoint1]) {
	//	throw TrackingException();
	//}
	...
	cvGetQuadrangleSubPix(originalImage, eyeImageLeft.get(), &M);
	cvCvtColor(eyeImageLeft.get(), eyeGreyLeft.get(), CV_RGB2GRAY);
}
